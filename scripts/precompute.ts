...
async function processFile(filePath: string, players: PlayerMap) {
    return new Promise<void>((resolve, reject) => {
        ...
        parser.on("data", (row: CsvRow) => {
            ...
            const batSpeed =
                row.bat_speed === "" || row.bat_speed == null
                    ? null
                    : Number(row.bat_speed);
            const swingLength =
            ...
            const balls =
                row.balls === "" || row.balls == null
                    ? null
                    : Number(row.balls);
            const strikes =
                row.strikes === "" || row.strikes == null
                    ? null
                    : Number(row.strikes);
...
function toMetricStats(run: RunningStats) {
    const batAvg =
        run.batSpeedCount > 0 ? run.batSpeedSum / run.batSpeedCount : null;
    const lenAvg =
        run.swingLengthCount > 0
            ? run.swingLengthSum / run.swingLengthCount
            : null;
    return {
...
async function main() {
    ...
    console.log(
        `Wrote aggregates for ${Object.keys(aggregates).length} hitters to ${outPath}`,
    );
}
